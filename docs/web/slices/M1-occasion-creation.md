...
## 2. Occasion kế thừa gì (không bắt user nhập lại)
- **Từ Synthesis (la bàn):** USP, wedge segment, giai đoạn roadmap đang active → mục
  tiêu định hướng của đợt.
- **Từ Tactical Playbook (T5):** cách đánh per-segment (copy angle, kênh, archetype).
- **Từ profile:** ngành (→ mùa vụ, archetype), ngân sách marketing/tháng (gợi ý ngân sách đợt).
